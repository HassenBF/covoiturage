...
$retour = mysql_query("SELECT COUNT(*) AS nbre_entrees FROM trajet WHERE ((heure >= '$heure1' AND heure <= '$heure2') OR heure='variable')  AND ville1 LIKE '%$ville1%' AND ville2 LIKE '%$ville2%' ");
	$donnees_compt = mysql_fetch_array($retour);	
	$i=$donnees_compt['nbre_entrees'];
	
$reponse = mysql_query("SELECT * FROM trajet WHERE ((heure >= '$heure1' AND heure <= '$heure2') OR heure='variable') AND ville1 LIKE '%$ville1%' AND ville2 LIKE '%$ville2%'  ") or die(mysql_error());		
